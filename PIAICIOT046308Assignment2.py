...
def function5():
    ...
    z = np.zeros(20).reshape(4,5).astype(np.int) #wrtie your code here
...
def function8():
    ...
    x = np.arange(10, dtype=np.int32).reshape((2,5)) #write your code here
    x[:] = 6
    return x
...
def function18():
    ...
    x = len(np.nonzero([np.where(piaic == students ) ])) # Write you code Here
    return x
